...
const readCaughtData = (data: Buffer) => {
    ...
    const locationAddress = Number.parseInt(binaryB.slice(1), 2)
        .toString(16)
        .toUpperCase();
...
const parsePokemon = (
    buf: Buffer,
    boxed = false,
): Gen2PokemonObject["pokemonList"][number] => {
